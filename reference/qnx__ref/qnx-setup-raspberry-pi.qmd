...
- Micro SD card, minimum size, 16 GB
- Micro SD card adapter (if necessary)
...
1. Navigate to the image.
   ...
   ```bash
   > cd ~/qnx800/images/rpi4
   ```

1. Run the script `unpack_rpi4_image.sh`

   ```bash
   > ./unpack_rpi4_image.sh
   ```

1. The image can be found in

   ```bash
   ~/qnx800/images/rpi4/rpi4/rpi4.img
   ```
...
scp user@192.168.0.x:~/path/to/rpi4.img
```

## Write QNX Image to SD Card

These are the steps I used to write the QNX image onto an SD card on a Macbook Pro from 2014. The steps will likely be similar on a Linux machine. Note that image file is 11 GB, so ensure you have an SD card that is large enough.
...
1. Write the `.img` file to the raw disk. This will overwrite the entire SD card.

   __Note:__ This step might take 1 - 2 _hours_. With my setup, it took 1h 5m 2s.

   ```bash
   # Note that on a linux system the value of bs= might need to be 4M (capital)
   > sudo dd if=/path/to/rpi4.img of=/dev/disk2 bs=4m
   ```

   The command will appear to hang. To view current progress of `dd` while it is running, from another terminal window, run:

   ```
   sudo kill -INFO $(pgrep ^dd$)
   ```

   Example output from `dd` window:

   ```bash
   363+0 records in
   362+0 records out
   1518338048 bytes transferred in 486.378867 secs (3121719 bytes/sec)
   ```

   To continuously print SD card write progress, you can run the command to print progress continuously every second. From a terminal window or pane different from the window running the `dd` command, execute the following loop:

   ```bash
   while true; do sudo kill -INFO $(pgrep ^dd$); sleep 1; done &
   ```
